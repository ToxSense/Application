...
class TFLiteClassifier(private val context: Context) {
    ...
    private var inputImageWidth: Int = 256
    private var inputImageHeight: Int = 256
    ...
    private fun initializeInterpreter() {
        ...
        Log.i(TAG, "initializeInterpreter: model file loaded ${model}")

        //labels = loadLines(context, "labels.txt")
        //labels = loadLines(context, "_labels.txt")
        ...
        Log.i(TAG, "initializeInterpreter: imgWidth ${inputImageWidth}, imgHeight ${inputImageHeight}")
    ...
    private fun classify(bitmap: Bitmap): Array<Any> {

        check(isInitialized) { "TF Lite Interpreter is not initialized yet." }
        val rImage =
            Bitmap.createScaledBitmap(bitmap, inputImageWidth, inputImageHeight, true)
        val resizedImage = rImage.rotate(getRotation(context)!!.toFloat() + 90.0f)
        ...
        val outputImg: Bitmap = getOutputImage(byteBuffer)
        ...
        //var index = output[0].roundToInt()
        var index = output

        return arrayOf(index[0][0], outputImg, inferenceTime)
        //return "Prediction is ${index[0][0]/1000}\nInference Time $inferenceTime ms"
    }

    fun classifyAsync(bitmap: Bitmap): Task<Array<Any>> {
        return call(executorService, Callable<Array<Any>> { classify(bitmap) })
    }
    ...
    private fun getOutputImage(output: ByteBuffer): Bitmap {
        ...
        var outputWidth = inputImageWidth
        var outputHeight = inputImageHeight
        ...
        for (i in 0 until outputWidth * outputHeight) {
            ...
            //Log.i(TAG, "getOutputImage: r:$r g:$g b:$b")
